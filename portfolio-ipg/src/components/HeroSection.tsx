...
import { heroContent } from "../contents/hero.content"; 
        ...
        <div className="flex flex-col md:flex-row items-center gap-12 md:gap-16">
          ...
          <div className="flex-1 text-center md:text-left space-y-6 animate-fade-in">
            <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-bold leading-tight tracking-tight group cursor-default">
              <span className="text-foreground inline-block transition-all duration-300 group-hover:scale-[1.03] group-hover:brightness-125">
                {heroContent.name.first}
              </span>{" "}
              <span className="text-primary inline-block transition-all duration-300 group-hover:scale-[1.03] group-hover:brightness-125">
            ...
            <div className="flex gap-3 justify-center md:justify-start pt-2">
              {heroContent.socialLinks.map((link) => {
                const Icon = iconsMap[link.label] || Mail;
          ...
          <div className="flex-shrink-0 animate-fade-in" style={{ animationDelay: "0.2s" }}>
            <div className="w-64 h-64 md:w-80 md:h-80 rounded-full overflow-hidden border-4 border-border shadow-2xl shadow-primary/10">
              <img
                src={profilePhoto}
                alt={`${heroContent.name.first} ${heroContent.name.last}`}
